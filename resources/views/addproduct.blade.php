    ...
    <div class="table-responsive">
        <table id="example" class="table table-hover" style="width:100%">
            ...
            <tbody>
                ...
                <tr>
                    <td><img width="50" height="50" src="{{ asset('images/'.$product->image) }}" alt="productimage"></td>
                    <td>{{$product->productName}}</td>
                    <td>{{$product->discountPrice}}</td>
                    <td>{{$product->species_name}}</td>
                    <td>{{$product->category_name}}</td>
                    <td>
                        <button title="Delete" class="btn btn-danger delete-button" data-product-id="{{ $product->id }}"><i class="fas fa-trash"></i></button>
                        <a href="{{ route('product.edit', $product->id) }}"><button title="Edit" class="btn btn-success"><i class="fas fa-edit"></i></button>
                       </a>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="productForm" action="{{ url('/products') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <!-- First Column -->
                        <div class="col-md-7">
                            ...
                            <div class="form-group">
                                <label for="productName">Plant Name</label>
                                <input type="text" class="form-control @error('productName') is-invalid @enderror" id="productName" name="productName" placeholder="Enter plant name" value="{{ old('productName') }}">
                                @error('productName')
                           ...
                           <!-- Discount -->
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control @error('description') is-invalid @enderror" id="description" name="description" rows="4" placeholder="Enter plant description">{{ old('description') }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
        ...
        Array.from(deleteButtons).forEach(button => {
            button.addEventListener('click', function () {
                ...
                if (confirm('Are you sure you want to delete this product?')) {
                    ...
                    form.method = 'POST';
                    form.action = `/addproduct/${productId}`;
